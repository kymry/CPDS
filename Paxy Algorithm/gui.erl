...
-define(WindowSize, {850, 1520}).
-define(PanelSize, {275, 100}).
-define(OuterSizerMinWidth, 190).
-define(OuterSizerMaxHeight, 420).	% maximum sizer size
-define(InSizerMinWidth, 275).
-define(InSizerMinHeight, 200).
...
start(Acceptors, Proposers) ->
    ...
    gui(State).
    %% no_window(Acceptors, Proposers).

no_window(Acceptors, Proposers) ->
    AccIds = lists:map(fun(_) ->
        spawn(fun() -> receive undefined -> ok end end)
    end, Acceptors),
    PropIds = lists:map(fun(_) ->
        spawn(fun() -> receive undefined -> ok end end)
    end, Proposers),
...
make_window(Acceptors, Proposers, AccPanelHeight, PropPanelHeight) ->
    ...
    % add spacers
    wxSizer:addSpacer(MainSizer, 50),  %spacer
    wxSizer:addSpacer(ProposerSizer, 25),
    wxSizer:addSpacer(AcceptorSizer, 25),
    ...
    wxSizer:addSpacer(MainSizer, 50),
    wxSizer:addSpacer(OuterSizer, 25),
